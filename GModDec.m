...
intrinsic QuoMap(M::GModDec, X::.) -> Mtrx
  ...
  return CompMapsToFullMap(Mnew, M, [ t[2] : t in quos]);
end intrinsic;
...
intrinsic IsCoercible(M::GModDec, x::.) -> BoolElt, GModDecElt
  ...
  if Type(x) eq GModDecElt and x in M then
    ...
    N := x`parent;
    Nbasis := [* [ Transpose(Matrix(u)) : u in Basis(U)] : U in N`subspaces *];
    xrows := [* Rows(M) : M in x`elt *];
    xx := [* #xrows[i] eq 0 select ZeroMatrix(BaseRing(M), M`multiplicities[i], Dimension(M`irreducibles[i])) else Matrix(&+[ Nbasis[i,j]*Matrix(xrows[i,j]) : j in [1..#xrows[i]]]) : i in [1..#N`irreducibles]*];
