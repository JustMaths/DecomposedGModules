...
intrinsic SubConstructor(M::GModDec, X::.) -> GModDec
  ...
  if #XX eq 0 then
    ...
    Mnew`subspaces := [ sub< Generic(M`subspaces[i]) | RowSequence(VerticalJoin([ Transpose(x`elt[i]) : x in XX])*BasisMatrix(M`subspaces[i]))> : i in [1..#M`irreducibles]];
  end if;
...
intrinsic IsCoercible(M::GModDec, x::.) -> BoolElt, GModDecElt
  ...
  if Type(x) eq GModDecElt and x in M then
    if x`parent eq M then
      return true, x;
    end if;
    N := x`parent;
    
    Nbasis := [* [ Transpose(Matrix(v)) : v in Basis(V)] : V in N`subspaces *];
    xrows := [* Rows(M) : M in x`elt *];
    // This is x coerced into Generic(M) (except with some zeroes)
    xx := [* #xrows[i] eq 0 select 0 else Matrix(&+[ Nbasis[i,j]*Matrix(xrows[i,j]) : j in [1..#xrows[i]]]) : i in [1..#N`irreducibles]*];
    
    // Find the representation with repsect to M
    Mmatsps := [* sub<KMatrixSpace(BaseRing(M), Dimension(Generic(M`subspaces[i])), Dimension(M`irreducibles[i])) | [ Transpose(Matrix(v))*Matrix(u) : v in Basis(M`subspaces[i]), u in Basis(VectorSpace(M`irreducibles[i]))]> : i in [1..#M`irreducibles]*];
    // We assume that the basis are matrices supported on only one column and have row rank 1 and are ordered as we think they should be.
    
    xxx := [* xx[i] eq 0 select ZeroMatrix(BaseRing(M), M`multiplicities[i], Dimension(M`irreducibles[i])) else Matrix(BaseRing(M), M`multiplicities[i], Dimension(M`irreducibles[i]), Coordinates(Mmatsps[i], Mmatsps[i]!xx[i])) : i in [1..#N`irreducibles]*];
    return true, CreateElement(M, xxx);
   ...
   then
    return true, CreateElement(M, x);
     ...
     (Type(x) eq ModTupFldElt and Degree(x) eq Dimension(M)) or
     (Type(x) eq ModGrpElt and Dimension(Parent(x)) eq Dimension(M)) then
