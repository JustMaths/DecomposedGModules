...
intrinsic SubConstructor(M::GModDec, X::.) -> GModDec, GModDecHom
  ...
  */
  ...
  incs := [ hom<Mnew`subspaces[i]-> M`subspaces[i] |
                  [ v@Morphism(Mnew`subspaces[i], M`subspaces[i]) : v in Basis(Mnew`subspaces[i])]>
             : i in [1..#M`irreducibles]];
...
intrinsic Morphism(N::GModDec, M::GModDec) -> GModDecHom
  ...
  return Hom(N, M, [* Morphism(N`subspaces[i], M`subspaces[i]) : i in [1..#M`irreducibles]*]);
end intrinsic;  
/*
...
intrinsic DirectSum(M::GModDec, N::GModDec) -> GModDec, AlgMatElt, AlgMatElt, AlgMatElt, AlgMatElt
  ...
  homM := [ hom<Msub -> VectorSpace(F, Dimension(Msub)) | Dimension(Msub) eq 0 select [] else IdentityMatrix(F, Dimension(Msub))> : Msub in M`subspaces];
  homN := [ hom<Nsub -> VectorSpace(F, Dimension(Nsub)) | Dimension(Nsub) eq 0 select [] else IdentityMatrix(F, Dimension(Nsub))> : Nsub in N`subspaces];
  
  Mnew`subspaces := [ VectorSpace(F, Dimension(t[1])) : t in sums ];
  Mnew`multiplicities := [ Dimension(V) : V in Mnew`subspaces];
  
  inj1 := Hom(M, Mnew, [sums[i,2]*homM[i] : i in [1..#sums]]);
  inj2 := Hom(N, Mnew, [t[3] : t in sums]);
  proj1 := Hom(Mnew, M, [t[4] : t in sums]);
  proj2 := Hom(Mnew, N, [t[5] : t in sums]);
...
intrinsic DirectSum(Q::SeqEnum[GModDec]) -> GModDec, SeqEnum, SeqEnum
  ...
  injs := [ Hom(Q[i], Mnew, [t[2,i] : t in sums]) : i in [1..#Q]];
  projs := [ Hom(Q[i], Mnew, [t[3,i] : t in sums]) : i in [1..#Q]];
  ...
  // make the injection and projection maps use GModDecHom
