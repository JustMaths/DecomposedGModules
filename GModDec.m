...
intrinsic SubConstructor(M::GModDec, X::.) -> GModDec
  ...
  if #X eq 1 and Type(X[1]) eq SeqEnum and forall{x : x in X[1] | Type(x) in { GModDec, SeqEnum, ModTupFldElt, ModGrpElt}} then
    X := X[1];
  end if;
  XX := { M | x : x in X };
...
intrinsic DirectSum(M::GModDec, N::GModDec) -> GModDec, AlgMatElt, AlgMatElt, AlgMatElt, AlgMatElt
  ...
  maps := [ DiagonalJoin(< MapToMatrix(t[i]) : t in sums >) : i in [2..5]];
...
intrinsic IsCoercible(M::GModDec, x::.) -> BoolElt, GModDecElt
  ...
  if Type(x) eq GModDecElt and x in M then
   ...
   then
    ...
    if Type(x) eq ModTupFldElt then
      x := Eltseq(x);
    end if;
